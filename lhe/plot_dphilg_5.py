...
#events = Events (['/eos/user/a/amlevin/data/miniaodsim/za_lowmll/s3_merge_ZA_lowmass_197092_0.root'])
...
#        continue
    ...
    for i in range(0,len(lheinfo.product().hepeup().IDUP)):
        ...
        if lheinfo.product().hepeup().IDUP.at(i) == 23:
            v_z.SetPxPyPzE(lheinfo.product().hepeup().PUP.at(i)[0],lheinfo.product().hepeup().PUP.at(i)[1],lheinfo.product().hepeup().PUP.at(i)[2],lheinfo.product().hepeup().PUP.at(i)[3])
...
#        if lheinfo.product().hepeup().IDUP.at(i) == -15:
    ...
    if v_lepton1.Pt() > 30 and v_photon.Pt() > 25 and v_lepton2.Pt() < 10:

        if v_z.M() != 0:
            v_lepton1.Boost(-v_z.BoostVector())
            if lheinfo.product().originalXWGTUP() > 0:
                h.Fill(v_lepton2.Pt())
            else:    
                h.Fill(v_lepton2.Pt(),-1)
