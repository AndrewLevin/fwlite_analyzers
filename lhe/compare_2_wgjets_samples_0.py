...
#events = Events(['/afs/cern.ch/work/a/amlevin/tmp/SMP-RunIISummer15wmLHEGS-00372.root'])
#xs = 7.637

events = Events(['/afs/cern.ch/work/a/amlevin/tmp/SMP-RunIISummer15wmLHEGS-00276.root'])
xs = 284.3
...
h_photon_pt=ROOT.TH1F("photon_pt","photon_pt",19,100,2000)
...
#    if count % 1 == 0:
    ...
    if lheinfo.product().originalXWGTUP() > 0:
        countweighted += 1
    else:
        countweighted -= 1
    ...
    for i in range(0,len(lheinfo.product().hepeup().IDUP)):
        if lheinfo.product().hepeup().IDUP.at(i) == 22:
            vphoton=ROOT.TLorentzVector();
            vphoton.SetPxPyPzE(lheinfo.product().hepeup().PUP.at(i)[0],lheinfo.product().hepeup().PUP.at(i)[1],lheinfo.product().hepeup().PUP.at(i)[2],lheinfo.product().hepeup().PUP.at(i)[3])
            found_photon = True
        if abs(lheinfo.product().hepeup().IDUP.at(i)) == 11 or abs(lheinfo.product().hepeup().IDUP.at(i)) == 13 or abs(lheinfo.product().hepeup().IDUP.at(i)) == 15:
            vlepton=ROOT.TLorentzVector();
            vlepton.SetPxPyPzE(lheinfo.product().hepeup().PUP.at(i)[0],lheinfo.product().hepeup().PUP.at(i)[1],lheinfo.product().hepeup().PUP.at(i)[2],lheinfo.product().hepeup().PUP.at(i)[3])
            found_lepton = True
       
    assert(found_photon and found_lepton)

    if vlepton.Pt() < 15:
        continue
     
    if lheinfo.product().originalXWGTUP() > 0:
        h_photon_pt.Fill(vphoton.Pt())
    else:
        h_photon_pt.Fill(vphoton.Pt(),-1)  

h_photon_pt.Scale(xs*1000*lumi/countweighted)

h_photon_pt.SetMinimum(0)

h_photon_pt.SetMaximum(4.0)
